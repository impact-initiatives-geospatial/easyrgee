...
`ee_extract_long` is a tool that can be used to extract values or zonal statistics over both Images and Image collections. It is a wrapper around `rgee::ee_extract` which does the same, but returns the output in wide format. `ee_extract_long` also contains several server-side processes and client-side regex manipulations to help standardize the output. The result is a data.frame ready for processing/plotting.
